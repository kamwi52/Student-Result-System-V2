<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
                        ...
                        <div class="hidden sm:flex sm:items-center sm:ms-6">
                            <x-dropdown align="left" width="48">
                                ...
                                <x-slot name="content">
                                    ...
                                    {{-- This link has been commented out to prevent the "Route not defined" error --}}
                                    {{-- <x-dropdown-link :href="route('admin.assignments.index')" :active="request()->routeIs('admin.assignments.*')">Manage Assignments</x-dropdown-link> --}}
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(Auth::user()->role === 'admin')
                <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
                    <div class="px-4"><div class="font-medium text-base text-gray-800 dark:text-gray-200">Admin Menu</div></div>
                    <div class="mt-3 space-y-1">
                        <x-responsive-nav-link :href="route('admin.users.index')">User Management</x-responsive-nav-link>
                        <x-responsive-nav-link :href="route('admin.subjects.index')">Subject Management</x-responsive-nav-link>
                        <x-responsive-nav-link :href="route('admin.classes.index')">Class Management</x-responsive-nav-link>
                        
                        {{-- This link has been commented out to prevent the "Route not defined" error --}}
                        {{-- <x-responsive-nav-link :href="route('admin.assignments.index')">Manage Assignments</x-responsive-nav-link> --}}
                        
                        <x-responsive-nav-link :href="route('admin.assessments.index')">Assessment Management</x-responsive-nav-link>
                        <x-responsive-nav-link :href="route('admin.results.index')">Result Management</x-responsive-nav-link>
                    </div>
                </div>
            @endif
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
            <div class="px-4"><div class="font-medium text-base">{{ Auth::user()->name }}</div><div class="font-medium text-sm">{{ Auth::user()->email }}</div></div>
            <div class="mt-3 space-y-1">
